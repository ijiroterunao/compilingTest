    ...
    for a in range(middle.__len__()):
        ...
        for b in range(exp.__len__()):
            ...
            if(type(item_or_plus) == type([])):#item
                print(item_or_plus)
                last_f_or_mul=item_or_plus[b-1]
                #print("last"+last_f_or_mul)
                next_f_or_mul=item_or_plus[b+1]
                #print("next"+next_f_or_mul)
                # c=0
                #print(range(item_or_plus.__len__()))
                for c in range(item_or_plus.__len__()):
                    ...
                    if(factor_or_mul)=='*':  #归约
                        ...
                        results.append("t"+str(nT)+" = "+item_or_plus[i])
                        item_or_plus[c]="t"+str(nT)
                        print("item_or_plus[c]"+item_or_plus[c])
                        nT+=1
                    elif item_or_plus[i] in Var_list:  # factor
                        #last_f_or_mul = f_or_mul
                        continue
                    elif isNumber(item_or_plus[i]):  # factor
                        #last_f_or_mul = f_or_mul
                        continue
